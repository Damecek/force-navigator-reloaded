/* global chrome */
import {
  APEX_CLASS_SETTINGS_KEY,
  APEX_TRIGGER_SETTINGS_KEY,
  AUTOLOGIN_SETTINGS_KEY,
  COMMAND_PALETTE_SETTINGS_KEY,
  COMMAND_PALETTE_SHOW_USAGE_SETTINGS_KEY,
  COMMANDS_SETTINGS_KEY,
  CUSTOM_METADATA_ENTITY_TYPE,
  ENTITY_DEFINITION_SETTINGS_KEY,
  EXPERIENCE_SITE_SETTINGS_KEY,
  FLOW_ACTIVE_VERSION_TYPE,
  FLOW_DEFINITION_SETTINGS_KEY,
  FLOW_DEFINITION_TYPE,
  FLOW_LATEST_VERSION_TYPE,
  GLOBAL_CACHE_SCOPE,
  LIGHTNING_APP_SETTINGS_KEY,
  PERMISSION_SET_GROUP_SETTINGS_KEY,
  PERMISSION_SET_SETTINGS_KEY,
  PERSONAL_SETTING_SETUP_NODE,
  LOGIN_AS_SETTINGS_KEY,
  REVIEW_COMMAND_ENABLED_SETTINGS_KEY,
  REVIEW_COMMAND_SETTINGS_KEY,
  SERVICE_SETUP_SETUP_NODE,
  SETUP_NODE_TYPES,
  SETUP_SETUP_NODE,
  SOBJECT_ENTITY_TYPE,
  USERS_SETTINGS_KEY,
} from './constants.js';
import {
  DEFAULT_OBJECT_SECTIONS,
  DEFAULT_FLOW_OPTIONS,
  DEFAULT_SOURCES,
} from '../navigator/defaults.js';
import CacheManager from './cacheManager.js';

/** In-memory settings cache. Null means cold (needs load from storage). */
let _settingsCache = null;

/**
 * Key used for persisting settings in chrome.storage.
 */
export const SETTINGS_KEY = 'settings';

/**
 * Default extension settings.
 */
export const DEFAULT_SETTINGS = {
  [AUTOLOGIN_SETTINGS_KEY]: false,
  [COMMAND_PALETTE_SETTINGS_KEY]: {
    [COMMAND_PALETTE_SHOW_USAGE_SETTINGS_KEY]: true,
  },
  [REVIEW_COMMAND_SETTINGS_KEY]: {
    [REVIEW_COMMAND_ENABLED_SETTINGS_KEY]: true,
  },
  [COMMANDS_SETTINGS_KEY]: {
    [SETUP_NODE_TYPES]: {
      [SETUP_SETUP_NODE]: true,
      [PERSONAL_SETTING_SETUP_NODE]: true,
      [SERVICE_SETUP_SETUP_NODE]: false,
    },
    [ENTITY_DEFINITION_SETTINGS_KEY]: {
      [CUSTOM_METADATA_ENTITY_TYPE]: true,
      [SOBJECT_ENTITY_TYPE]: { ...DEFAULT_OBJECT_SECTIONS },
    },
    [FLOW_DEFINITION_SETTINGS_KEY]: {
      [FLOW_DEFINITION_TYPE]: DEFAULT_FLOW_OPTIONS.includeDefinition,
      [FLOW_LATEST_VERSION_TYPE]: DEFAULT_FLOW_OPTIONS.includeLatest,
      [FLOW_ACTIVE_VERSION_TYPE]: DEFAULT_FLOW_OPTIONS.includeActive,
    },
    [APEX_CLASS_SETTINGS_KEY]: DEFAULT_SOURCES.includes('apex-classes'),
    [APEX_TRIGGER_SETTINGS_KEY]: DEFAULT_SOURCES.includes('apex-triggers'),
    [EXPERIENCE_SITE_SETTINGS_KEY]:
      DEFAULT_SOURCES.includes('experience-sites'),
    [LIGHTNING_APP_SETTINGS_KEY]: DEFAULT_SOURCES.includes('apps'),
    [PERMISSION_SET_SETTINGS_KEY]: DEFAULT_SOURCES.includes('permission-sets'),
    [PERMISSION_SET_GROUP_SETTINGS_KEY]: DEFAULT_SOURCES.includes(
      'permission-set-groups'
    ),
    [USERS_SETTINGS_KEY]: DEFAULT_SOURCES.includes('users'),
    [LOGIN_AS_SETTINGS_KEY]: true,
  },
};

function mergeSettings(partial) {
  const merged = JSON.parse(JSON.stringify(DEFAULT_SETTINGS));

  function recursiveAssign(target, source) {
    for (const key in source) {
      if (Object.prototype.hasOwnProperty.call(source, key)) {
        if (
          source[key] &&
          typeof source[key] === 'object' &&
          !Array.isArray(source[key]) &&
          target[key] &&
          typeof target[key] === 'object' &&
          !Array.isArray(target[key])
        ) {
          recursiveAssign(target[key], source[key]);
        } else {
          target[key] = source[key];
        }
      }
    }
  }

  if (partial && typeof partial === 'object') {
    recursiveAssign(merged, partial);
  }
  console.log('Merged settings', merged);
  return merged;
}

/**
 * Load settings from storage. Missing values are filled with defaults.
 * @returns {Promise<typeof DEFAULT_SETTINGS>}
 */
export async function loadSettings() {
  if (_settingsCache !== null) {
    return _settingsCache;
  }
  const stored = await new CacheManager(GLOBAL_CACHE_SCOPE).get(SETTINGS_KEY);
  console.log('Loading settings', stored);
  const merged = mergeSettings(stored);
  if (JSON.stringify(stored) !== JSON.stringify(merged)) {
    await new CacheManager(GLOBAL_CACHE_SCOPE).set(SETTINGS_KEY, merged, {
      preserve: true,
    });
  }
  _settingsCache = merged;
  return merged;
}

/**
 * Save provided settings to storage.
 * @param {typeof DEFAULT_SETTINGS} settings
 * @return {Promise<typeof DEFAULT_SETTINGS>}
 */
export async function saveSettings(settings) {
  console.log('Saving settings', settings);
  const merged = mergeSettings(settings);
  await new CacheManager(GLOBAL_CACHE_SCOPE).set(SETTINGS_KEY, merged, {
    preserve: true,
  });
  _settingsCache = merged;
  return merged;
}

/**
 * Reset user settings to defaults.
 * @returns {Promise<typeof DEFAULT_SETTINGS>}
 */
export async function resetSettings() {
  console.log('Resetting settings to defaults');
  _settingsCache = null;
  await new CacheManager(GLOBAL_CACHE_SCOPE).clear(SETTINGS_KEY);
  return loadSettings();
}

/**
 * Get value from settings by key.
 * @param {string[]} keyPath
 * @return {Promise<any>}
 */
export async function getSetting(keyPath) {
  console.log('Getting settings for key', keyPath);
  const settings = await loadSettings();
  const value = keyPath.reduce((acc, key) => {
    if (acc && Object.prototype.hasOwnProperty.call(acc, key)) {
      return acc[key];
    }
    return undefined;
  }, settings);
  console.log('Setting value for key', keyPath, value);
  return value;
}

/**
 * Invalidate the in-memory settings cache so the next read reloads from storage.
 */
export function invalidateSettingsCache() {
  _settingsCache = null;
}

/**
 * Returns whether My Domain auto-login is enabled.
 * @returns {Promise<boolean>}
 */
export async function isAutologinEnabled() {
  return (await getSetting([AUTOLOGIN_SETTINGS_KEY])) === true;
}
